...
class EscapeRoomCommandHandler:
    ...
    def _cmd_look(self, look_args):
        ...
        if len(look_args) == 0:
            ...
            object = self.room["container"].get(
                look_args[-1], self.player["container"].get(look_args[-1], None))
        ...
        if not object or not object["visible"]:
            ...
            if not object["open"]:
                ...
                look_result = "Inside the {} you see: {}".format(
                    object.name, listFormat(object["container"].values()))
        else:
            self._run_triggers(object, "look")
            look_result = object.attributes.get(
    ...
    def _cmd_unlock(self, unlock_args):
        ...
        if len(unlock_args) == 0:
            ...
            if not object or not object["visible"]:
                ...
                if not unlocker:
                    unlock_result = "You don't have a {}".format(
                        unlock_args[-1])
    ...
    def _cmd_get(self, get_args):
        if len(get_args) == 0:
            ...
            if len(get_args) > 1:
                ...
                container = self.room
            object = container["container"] and container["container"].get(
                get_args[0], None) or None
    ...
    def _cmd_hit(self, hit_args):
        ...
        if not target["hittable"]:
            ...
            self.output("You hit the {} with the {}".format(
                target_name, with_what_name))
...
class EscapeRoomGame:
    def __init__(self, command_handler_class=EscapeRoomCommandHandler, output=print, conn=None):
    ...
    def create_game(self, cheat=False, conn=None):
        ...
        key = EscapeRoomObject("key",    visible=True,
                               gettable=True, interesting=True)
        door = EscapeRoomObject("door",   visible=True, openable=True,
                                open=False, keyed=True, locked=True, unlockers=[key])
        chest = EscapeRoomObject("chest",  visible=True, openable=True,
                                 open=False, keyed=True, locked=True, unlockers=[hairpin])
        ...
        flyingkey = EscapeRoomObject("flyingkey", visible=True, flying=True, hittable=False, smashers=[
                                     hammer], interesting=True, location="ceiling")

        # setup containers
        player["container"] = {}
        chest["container"] = create_container_contents(hammer)
        room["container"] = create_container_contents(
            player, door, clock, mirror, hairpin, chest, flyingkey)
        ...
        flyingkey["short_description"] = create_flyingkey_short_description(
            flyingkey)
        ...
        mirror.triggers.append(
            lambda obj, cmd, *args: (cmd == "look") and hairpin.__setitem__("visible", True))
        mirror.triggers.append(lambda obj, cmd, *args: (cmd == "look") and mirror.__setitem__(
            "description", create_mirror_description(mirror, room)))
        door.triggers.append(lambda obj, cmd, *args: (cmd == "unlock")
                             and door.__setitem__("description", create_door_description(door)))
        door.triggers.append(lambda obj, cmd, *args: (cmd == "open")
                             and room["container"].__delitem__(player.name))
        room.triggers.append(lambda obj, cmd, *args: (cmd ==
                                                      "_post_command_") and advance_time(room, clock))
        flyingkey.triggers.append((lambda obj, cmd, *args: (
            cmd == "hit" and args[0] in obj["smashers"]) and flyingkey_hit_trigger(room, flyingkey, key, self.output)))
        ...
        self.conn = conn
        self.output = self.outputConn
        self.room, self.player = room, player
        self.command_handler = self.command_handler_class(
            room, player, self.output)
        self.agents.append(self.flyingkey_agent(flyingkey))
        self.status = "created"

    def outputConn(self, string):
        output_string = string + '<EOL>\n'
        data = output_string.encode()
    ...
    def move_flyingkey(self, flyingkey):
        ...
        self.output("The {} flies from the {} to the {}".format(
            flyingkey.name, old_location, next_location))
    ...
    def start(self):
        ...
        self.status = "playing"
        self.output(
            "Where are you? You don't know how you got here... Were you kidnapped? Better take a look around")
